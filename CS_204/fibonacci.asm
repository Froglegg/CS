...
outNewLine:  lw   $a0, 0($t0)      # load fibonacci number for syscall
      ...
      addi $t1, $t1, -1     # decrement loop counter
      bgt  $t1, 2, outNewLine # repeat if not finished
      bgtz $t1, outSpace
 
 outSpace: lw   $a0, 0($t0)      # load fibonacci number for syscall
      li   $v0, 1           # specify Print Integer service
      syscall               # print fibonacci number	
      ...
      addi $t0, $t0, 4      # increment address
      addi $t1, $t1, -1     # decrement loop counter
      bgtz $t1, outSpace    # repeat if not finished
      jr   $ra              # return